...
| File | Change |
|---|---|
| `path/to/file.ts` | Past-tense verb first, symbol in backticks, one or two sentences: what it does and what it enables or prevents. |
...
| Files table | One row per file, or per group changed for one reason (generated code, locales, lockfiles, fixtures). The Change cell reads like a GitHub Copilot summary item: `Added a pure builder that…`, `Replaced the per-page call with…`. | up to 25 words per cell |
...
enforced by the guard anywhere in the body outside inline code, and its
`ci-checks` block under `## How to verify`. A repository
...
reads it and does not expand shell variables). `glab` has no body-file flag, so
the guard reads the file named in `-d "$(cat <path>)"`. Read `example.md` and `voice.md` in
...
| A cell that lists every detail of the file | The one fact a reviewer needs before opening it. The rest is in the diff |
